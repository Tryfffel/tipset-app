...
const BOT_URL = "https://tryfffel.github.io/tipset-app/data/current.json";

// ── Fallback (används om fetch misslyckas) ────────────────────────────────────
...
function normalizeCoupons(data) {
  // Stödjer både ett objekt och en array
  const arr = Array.isArray(data) ? data : [data];
  return arr.filter(c => c && c.matches);
}
...
function productLabel(p) {
  if (!p) return "Tipset";
  return p === "europatipset" ? "Europatipset" : p === "stryktipset" ? "Stryktipset" : p;
}

function productColor(p) {
  return p === "europatipset" ? "#3b82f6" : "#8b5cf6";
}
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#e5e7eb" }}>
      <div style={{ fontWeight: 600, marginBottom: 6 }}>{label} · {productLabel(d.product)}</div>
    ...
    <div style={{ display: "flex", gap: 3, alignItems: "center" }}>
      {[a, b, c].map((v, i) => (
        <div key={i} style={{ textAlign: "center", minWidth: 28 }}>
          <div style={{ height: 3, borderRadius: 2, marginBottom: 3, background: v === max ? "#3b82f6" : "#2a2a2a", width: `${v}%`, maxWidth: 28, opacity: v === max ? 1 : 0.5 }} />
          <span style={{ fontSize: 11, color: v === max ? "#e5e7eb" : "#4b5563", fontVariantNumeric: "tabular-nums" }}>{v}</span>
        </div>
      ))}
    </div>
  );
...
function CouponView({ coupon, onSave }) {
  const [matches, setMatches] = useState(() =>
    coupon.matches.map(m => ({ ...m, sel: parseRec(m.recommendation) }))
  );

  // Reset when coupon changes (switching between coupons)
  useEffect(() => {
    setMatches(coupon.matches.map(m => ({ ...m, sel: parseRec(m.recommendation) })));
  }, [coupon.draw_number, coupon.product]);
  ...
  return (
    <div>
      ...
      {/* Footer */}
      <div style={{ margin: "16px 0 0", background: "#111", border: "1px solid #1c1c1c", borderRadius: 12, padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
          ...
          <div style={{ width: 1, background: "#1c1c1c", height: 32 }} />
          <button
            onClick={() => onSave({ coupon, matches, totalRows, systemStr })}
            style={{ padding: "12px 24px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 14, fontWeight: 700, background: "linear-gradient(135deg, #3b82f6, #6366f1)", color: "#fff", boxShadow: "0 0 20px rgba(59,130,246,0.3)", whiteSpace: "nowrap" }}
          >
            Spara och logga omgång
          </button>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  const [activeCoupon, setActiveCoupon] = useState(0); // index in coupons[]
  ...
  // ── Fetch coupons
  useEffect(() => {
    fetch(BOT_URL + "?t=" + Date.now()) // bust cache
      .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
      .then(data => {
        const normalized = normalizeCoupons(data);
        if (normalized.length === 0) throw new Error("Tom JSON");
  ...
  // ── Save round
  const saveRound = ({ coupon, matches, totalRows, systemStr }) => {
    const key = `${coupon.product}-${coupon.draw_number}`;
    if (history.some(h => h.key === key)) {
      showToast(`${productLabel(coupon.product)} #${coupon.draw_number} är redan sparad`, "warning");
      return;
    }
    const entry = {
      id: Date.now(),
    ...
    showToast(`${productLabel(coupon.product)} #${coupon.draw_number} sparad — ${totalRows} rader · ${fmtSEK(totalRows)}`);
  };

  // ── Edit winnings
  const startEdit = (id, val) => { setEditingId(id); setEditVal(String(val)); };
  const commitEdit = (id) => {
  ...
  const chartData = [...filtered].reverse().map(h => ({ round: `#${h.round}`, product: h.product, cost: h.cost, winnings: h.winnings, roi: roiPct(h.cost, h.winnings) }));
  ...
  // ── Render
  return (
    <div style={{ background: "#0d0d0d", minHeight: "100vh", color: "#e5e7eb", fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ padding: "16px 24px", borderBottom: "1px solid #1c1c1c", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 16, fontWeight: 800, letterSpacing: -0.5 }}>Tipset</span>
          {loading && <span style={{ fontSize: 11, color: "#4b5563" }}>Hämtar…</span>}
        </div>
        <div style={{ display: "flex", gap: 2, background: "#161616", borderRadius: 8, padding: 3 }}>
          {["coupon", "history"].map(v => (
            <button key={v} onClick={() => setView(v)} style={{ padding: "7px 16px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 500, transition: "all 0.15s", background: view === v ? "#252525" : "transparent", color: view === v ? "#f1f5f9" : "#4b5563" }}>
              {v === "coupon" ? "Kupong" : <>Historik {history.length > 0 && <span style={{ marginLeft: 5, fontSize: 10, fontWeight: 700, background: "#3b82f622", color: "#3b82f6", padding: "1px 5px", borderRadius: 10 }}>{history.length}</span>}</>}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Fetch status banner */}
      {!loading && isFallback && (
        <div style={{ padding: "8px 24px", background: "#1c0e00", borderBottom: "1px solid #3b1a00", fontSize: 11, color: "#fb923c" }}>
          ⚠ Kunde inte nå {BOT_URL} ({fetchError}) — visar fallback-data
        </div>
      )}
      {!loading && !isFallback && (
        <div style={{ padding: "6px 24px", background: "#0a160a", borderBottom: "1px solid #14532d", fontSize: 11, color: "#4ade80", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span>✓ Data hämtad från boten — {coupons.length} kupong{coupons.length !== 1 ? "er" : ""}</span>
          <span style={{ color: "#1a3a1a" }}>{BOT_URL}</span>
        </div>
      )}
      ...
      {view === "coupon" && !loading && (
        <>
          {/* Coupon selector (visas bara om det finns flera) */}
          {coupons.length > 1 && (
            <div style={{ padding: "12px 24px", borderBottom: "1px solid #161616", display: "flex", gap: 8, alignItems: "center" }}>
              <span style={{ fontSize: 11, color: "#3b3b3b", marginRight: 4 }}>Välj kupong:</span>
              {coupons.map((c, i) => (
                <button key={i} onClick={() => setActiveCoupon(i)} style={{
                  padding: "6px 14px", borderRadius: 6, border: "none", cursor: "pointer",
                  fontSize: 12, fontWeight: 600,
                  background: activeCoupon === i ? productColor(c.product) + "22" : "#161616",
                  color: activeCoupon === i ? productColor(c.product) : "#4b5563",
                  outline: activeCoupon === i ? `1px solid ${productColor(c.product)}44` : "none",
                }}>
                  {productLabel(c.product)} <span style={{ opacity: 0.6 }}>#{c.draw_number}</span>
                </button>
              ))}
            </div>
          )}

          {/* Single coupon header (visas alltid) */}
          {coupons.length > 0 && (
            <div style={{ padding: "8px 24px", borderBottom: "1px solid #161616", display: "flex", gap: 20, alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginRight: 8 }}>
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", color: productColor(coupons[activeCoupon]?.product), background: productColor(coupons[activeCoupon]?.product) + "18", padding: "3px 8px", borderRadius: 4 }}>
                  {productLabel(coupons[activeCoupon]?.product)}
                </span>
                <span style={{ fontSize: 13, fontWeight: 600 }}>#{coupons[activeCoupon]?.draw_number}</span>
              </div>
              <div style={{ width: 1, background: "#1c1c1c", height: 16 }} />
          ...
          <div style={{ padding: "0 24px 40px" }}>
            {coupons.length > 0
              ? <CouponView key={`${coupons[activeCoupon].product}-${coupons[activeCoupon].draw_number}`} coupon={coupons[activeCoupon]} onSave={saveRound} />
              : <div style={{ padding: 40, textAlign: "center", color: "#3b3b3b" }}>Inga kuponger tillgängliga</div>
            }
          </div>
        </>
      )}
        ...
        <div style={{ padding: "0 24px 40px" }}>
          ...
          {history.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px", color: "#3b3b3b", fontSize: 13 }}>
              ...
              <div>Öppna en kupong och klicka <strong style={{ color: "#3b82f6" }}>Spara och logga omgång</strong></div>
            </div>
          ) : (
            <>
              {/* Stat cards */}
              <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
                {[
                  ...
                  <div key={label} style={{ background: "#111", border: "1px solid #1c1c1c", borderRadius: 10, padding: "14px 18px", flex: 1 }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 1.2, color: "#3b3b3b", fontWeight: 600 }}>
                    <th style={{ ...td, textAlign: "left" }}>Omgång</th>
                    <th style={{ ...td, textAlign: "left" }}>Produkt</th>
                    <th style={{ ...td, textAlign: "left" }}>Datum</th>
                    <th style={{ ...td, textAlign: "left" }}>System</th>
                    <th style={{ ...td, textAlign: "right" }}>Insats</th>
                    <th style={{ ...td, textAlign: "right" }}>Vinst</th>
                    <th style={{ ...td, textAlign: "right" }}>ROI</th>
                    <th style={{ ...td, width: 32 }} />
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((h) => {
                    ...
                    return (
                      <tr key={h.id} style={{ borderTop: "1px solid #161616" }}>
                        <td style={{ ...td, fontSize: 13, fontWeight: 600 }}>#{h.round}</td>
                        <td style={td}>
                          <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: productColor(h.product) + "22", color: productColor(h.product) }}>
                            {h.product === "europatipset" ? "Europa" : "Stryk"}
                          </span>
                        </td>
                        <td style={{ ...td, fontSize: 12, color: "#4b5563" }}>{h.date}</td>
                        <td style={{ ...td, fontSize: 11, fontFamily: "monospace", color: "#4b5563" }}>{h.system}</td>
                        <td style={{ ...td, textAlign: "right", fontSize: 12, color: "#6b7280" }}>{fmtSEK(h.cost)}</td>
                        ...
                        <td style={{ ...td, textAlign: "right" }}>
                          <button onClick={() => deleteRound(h.id)}
